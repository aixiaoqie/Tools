...
public class SwaggerConfig {
	...
	public Docket gpApi() {
		...
		Predicate<String> or = or(regex("/extend/swagger/gp/.*"));
	...
	public Docket srApi() {
		@SuppressWarnings("unchecked")
		Predicate<String> or = or(regex("/extend/swagger/sr/.*"));
		return new Docket(DocumentationType.SWAGGER_2).groupName("SrApiInfo").apiInfo(srApiInfo()).select().paths(or).build()//
				.genericModelSubstitutes(DeferredResult.class)//
				.forCodeGeneration(false)//
				.globalOperationParameters(setHeaderToken())
				.pathMapping("/");
	...
	private ApiInfo srApiInfo() {
		return new ApiInfo("服务资源目录（sr）", // 标题
				"服务资源目录相关表的API文档", // 描述
				"Web Portals", // 版本
				"", new Contact("Zee", "", ""), // 作者
				"", // 许可
