...
import com.run.dao.unity.gp.IGpTokenUntDal;
 ...
 * @description token信息。 业务逻辑处理类，扩展自BaseUntBll<GpToken>，自动生成。
 */
public class GpTokenGenUntBll extends BaseUntBll<GpToken> {

	@Autowired
	protected IGpTokenUntDal gpTokenUntDal;


	public ResultModel updateList(GpTokenParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpTokenParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpTokenParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpTokenParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpTokenParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpTokenParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpTokenParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpToken> list = baseUntDal.getList(map);
			PageInfo<GpToken> pageInfo = new PageInfo<GpToken>(list);
			List<GpToken> modelList = pageInfo.getList();
