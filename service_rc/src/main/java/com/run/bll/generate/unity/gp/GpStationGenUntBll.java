...
import com.run.dao.unity.gp.IGpStationUntDal;
import com.run.ent.custom.ResultModel;
import com.run.ent.extend.gp.GpStation;
import com.run.ent.parameter.gp.GpStationParameter;
...
public class GpStationGenUntBll extends BaseUntBll<GpStation> {
	...
	@Autowired
	protected IGpStationUntDal gpStationUntDal;


	public ResultModel updateList(GpStationParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpStationParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpStationParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpStationParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpStationParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpStationParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpStationParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpStation> list = baseUntDal.getList(map);
			PageInfo<GpStation> pageInfo = new PageInfo<GpStation>(list);
			List<GpStation> modelList = pageInfo.getList();
