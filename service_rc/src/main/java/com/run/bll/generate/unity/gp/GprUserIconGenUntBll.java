...
import com.run.dao.unity.gp.IGprUserIconUntDal;
 ...
 * @description 用户头像。 业务逻辑处理类，扩展自BaseUntBll<GprUserIcon>，自动生成。
 */
public class GprUserIconGenUntBll extends BaseUntBll<GprUserIcon> {

	@Autowired
	protected IGprUserIconUntDal gprUserIconUntDal;


	public ResultModel updateList(GprUserIconParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GprUserIconParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GprUserIconParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GprUserIconParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GprUserIconParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GprUserIconParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GprUserIconParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GprUserIcon> list = baseUntDal.getList(map);
			PageInfo<GprUserIcon> pageInfo = new PageInfo<GprUserIcon>(list);
			List<GprUserIcon> modelList = pageInfo.getList();
