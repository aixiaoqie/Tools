...
import com.run.dao.unity.sr.ISrResourceUntDal;
import com.run.ent.custom.ResultModel;
import com.run.ent.extend.sr.SrResource;
import com.run.ent.parameter.sr.SrResourceParameter;
 ...
 * @description 服务资源 业务逻辑处理类，扩展自BaseUntBll<SrResource>，自动生成。
 */
public class SrResourceGenUntBll extends BaseUntBll<SrResource> {

	@Autowired
	protected ISrResourceUntDal srResourceUntDal;


	public ResultModel updateList(SrResourceParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(SrResourceParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(SrResourceParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(SrResourceParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			SrResourceParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			SrResourceParameter.GetList.Page page = getListParam.getPage();
			ArrayList<SrResourceParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<SrResource> list = baseUntDal.getList(map);
			PageInfo<SrResource> pageInfo = new PageInfo<SrResource>(list);
			List<SrResource> modelList = pageInfo.getList();
