...
import com.run.dao.unity.gp.IGpLoginLogUntDal;
 ...
 * @description 登录日志。 业务逻辑处理类，扩展自BaseUntBll<GpLoginLog>，自动生成。
 */
public class GpLoginLogGenUntBll extends BaseUntBll<GpLoginLog> {

	@Autowired
	protected IGpLoginLogUntDal gpLoginLogUntDal;


	public ResultModel updateList(GpLoginLogParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpLoginLogParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpLoginLogParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpLoginLogParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpLoginLogParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpLoginLogParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpLoginLogParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpLoginLog> list = baseUntDal.getList(map);
			PageInfo<GpLoginLog> pageInfo = new PageInfo<GpLoginLog>(list);
			List<GpLoginLog> modelList = pageInfo.getList();
