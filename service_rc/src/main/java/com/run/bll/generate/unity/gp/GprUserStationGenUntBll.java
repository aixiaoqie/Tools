...
import com.run.dao.unity.gp.IGprUserStationUntDal;
import com.run.ent.custom.ResultModel;
import com.run.ent.extend.gp.GprUserStation;
import com.run.ent.parameter.gp.GprUserStationParameter;
 ...
 * @description 用户所属岗位。 业务逻辑处理类，扩展自BaseUntBll<GprUserStation>，自动生成。
 */
public class GprUserStationGenUntBll extends BaseUntBll<GprUserStation> {

	@Autowired
	protected IGprUserStationUntDal gprUserStationUntDal;


	public ResultModel updateList(GprUserStationParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GprUserStationParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GprUserStationParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GprUserStationParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GprUserStationParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GprUserStationParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GprUserStationParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GprUserStation> list = baseUntDal.getList(map);
			PageInfo<GprUserStation> pageInfo = new PageInfo<GprUserStation>(list);
			List<GprUserStation> modelList = pageInfo.getList();
