...
import com.run.dao.unity.gp.IGpOperLogUntDal;
...
public class GpOperLogGenUntBll extends BaseUntBll<GpOperLog> {
	...
	@Autowired
	protected IGpOperLogUntDal gpOperLogUntDal;


	public ResultModel updateList(GpOperLogParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpOperLogParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpOperLogParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpOperLogParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpOperLogParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpOperLogParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpOperLogParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpOperLog> list = baseUntDal.getList(map);
			PageInfo<GpOperLog> pageInfo = new PageInfo<GpOperLog>(list);
			List<GpOperLog> modelList = pageInfo.getList();
