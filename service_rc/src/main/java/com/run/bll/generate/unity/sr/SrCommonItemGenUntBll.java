...
import com.run.dao.unity.sr.ISrCommonItemUntDal;
 ...
 * @description 公共数据项 业务逻辑处理类，扩展自BaseUntBll<SrCommonItem>，自动生成。
 */
public class SrCommonItemGenUntBll extends BaseUntBll<SrCommonItem> {

	@Autowired
	protected ISrCommonItemUntDal srCommonItemUntDal;


	public ResultModel updateList(SrCommonItemParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(SrCommonItemParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(SrCommonItemParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(SrCommonItemParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			SrCommonItemParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			SrCommonItemParameter.GetList.Page page = getListParam.getPage();
			ArrayList<SrCommonItemParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<SrCommonItem> list = baseUntDal.getList(map);
			PageInfo<SrCommonItem> pageInfo = new PageInfo<SrCommonItem>(list);
			List<SrCommonItem> modelList = pageInfo.getList();
