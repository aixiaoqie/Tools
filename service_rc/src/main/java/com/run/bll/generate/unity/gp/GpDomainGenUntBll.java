...
import com.run.dao.unity.gp.IGpDomainUntDal;
...
public class GpDomainGenUntBll extends BaseUntBll<GpDomain> {
	...
	@Autowired
	protected IGpDomainUntDal gpDomainUntDal;


	public ResultModel updateList(GpDomainParameter.UpdateList updateListParam) {
		return updateList(updateListParam, isLogWrite);
	}

	public ResultModel updateList(GpDomainParameter.UpdateList updateListParam, boolean isLog) {
	...
	public ResultModel getList(GpDomainParameter.GetList getListParam) {
		return getList(getListParam, isLogRead);
	}

	public ResultModel getList(GpDomainParameter.GetList getListParam, boolean isLog) {
		ResultModel result = new ResultModel();
		...
		try {
			...
			GpDomainParameter.GetList.EntityRelated entityRelated = getListParam.getEntityRelated();
			GpDomainParameter.GetList.Page page = getListParam.getPage();
			ArrayList<GpDomainParameter.GetList.Order> orderList = getListParam.getOrderList();
			...
			List<GpDomain> list = baseUntDal.getList(map);
			PageInfo<GpDomain> pageInfo = new PageInfo<GpDomain>(list);
			List<GpDomain> modelList = pageInfo.getList();
